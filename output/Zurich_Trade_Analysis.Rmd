...
### Strategic Synthesis and Systemic Conclusion
...
### Regional Symbiosis and the European Hub Effect
...
### Global Specialization and Industrial Sovereignty
...
## The Meta-Analysis of Zurich’s Industrial DNA (2016–2024)
...
```{r plot_10_waffle_dna, fig.width=6.5, fig.height=3.5, fig.align='center', message=FALSE, warning=FALSE}
library(tidyverse)

# 1. Compact Data Preparation
# Filter for milestone years and major continents to save space
waffle_prep <- df %>%
  filter(Year %in% c(2016, 2024), 
         Trade_Type == "export",
         Continent %in% c("Europe", "Asia", "Americas")) %>%
  mutate(Ind_Short = recode(Industry, 
    "Pharmazeutische Erzeugnisse" = "Pharma",
    "Maschinen" = "Mach.",
    "Chemische Erzeugnisse" = "Chem.",
    "Elektrische Ausrüstungen" = "E-Eq",
...
# Calculate 10x10 Grid Tiles (Percentage)
waffle_data <- waffle_prep %>%
  group_by(Year, Continent, Ind_Short) %>%
  summarise(Total = sum(Total_Amount_CHF, na.rm = TRUE), .groups = "drop_last") %>%
  mutate(Tiles = round(Total / sum(Total) * 100)) %>%
  uncount(Tiles) %>%
  group_by(Year, Continent) %>%
  slice_head(n = 100) %>% # Ensure exactly 100 tiles
  mutate(x = rep(1:10, length.out = n()),
         y = rep(1:10, each = 10, length.out = n()))

# 2. Compact Waffle Plot
ggplot(waffle_data, aes(x = x, y = y, fill = Ind_Short)) +
  ...
  theme_void() +
  labs(title = "Industrial Export DNA: 2016 vs 2024",
       subtitle = "1 Tile = 1% of Continental Export Volume",
       fill = "Sector") +
  theme(
...
### Conceptual Rationale and Zero-Sum Modelling
...
### Methodological Implementation and Seat Allocation
...
### Structural Revelations: Industrial Colonisation vs. Mosaic Resilience
...
### Synthesis and the Golden Cage Conclusion
